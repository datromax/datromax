...
PACKAGE_VERSION=$($SRC_PATH/src/datromaxd --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/datromaxd $DEB_BIN
cp $SRC_PATH/src/datromax-cli $DEB_BIN
cp $SRC_PATH/util/fetch-params.sh $DEB_BIN/datromax-fetch-params
# Copy docs
...
# Copy manpages
cp $SRC_DOC/man/datromaxd.1 $DEB_MAN
cp $SRC_DOC/man/datromax-cli.1 $DEB_MAN
cp $SRC_DOC/man/datromax-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/datromaxd.bash-completion $DEB_CMP/datromaxd
cp $SRC_PATH/contrib/datromax-cli.bash-completion $DEB_CMP/datromax-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/datromaxd.1
gzip --best -n $DEB_MAN/datromax-cli.1
gzip --best -n $DEB_MAN/datromax-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/datromaxd $DEB_BIN/datromax-cli
