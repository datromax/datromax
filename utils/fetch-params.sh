...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/DatromaxParams"
else
    PARAMS_DIR="$HOME/.datromax-params"
fi
...
DOWNLOAD_URL="https://github.com/datromax/datromax/releases/tag/1.2.15.3"
...
# fetch methods can be disabled with DTRX_DISABLE_SOMETHING=1
DTRX_DISABLE_WGET="${DTRX_DISABLE_WGET:-}"
#DTRX_DISABLE_IPFS="${DTRX_DISABLE_IPFS:-}"
DTRX_DISABLE_CURL="${DTRX_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$DTRX_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFSCMD" ] || ! [ -z "$DTRX_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$DTRX_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the Datromax parameters!
...
main() {
    ...
    cat <<EOF
DTRX - fetch-params.sh

This script will fetch the Datromax SNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Datromax SNARK parameters. Note that it is
