...
const char * const BITCOIN_CONF_FILENAME = "datromax.conf";
const char * const BITCOIN_PID_FILENAME = "datromaxd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "datromaxd -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\DatromaxCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\DatromaxCore
    // Mac: ~/Library/Application Support/DatromaxCore
    // Unix: ~/.datromaxcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "DatromaxCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/DatromaxCore";
#else
    // Unix
    return pathRet / ".datromaxcore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty datromax.conf if it does not excist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Datromax Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2021, nEndYear) + "The Datromax Core developers";
    }
