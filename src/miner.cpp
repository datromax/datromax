...
void static DatromaxMiner(const CChainParams& chainparams)
{
    LogPrintf("DatromaxMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("datromax-miner");
    ...
    if (!EnsureWalletIsAvailable(pWallet, false)) {
        LogPrintf("DatromaxMiner -- Wallet not available\n");
    }
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("DatromaxMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            ...
            LogPrintf("DatromaxMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("DatromaxMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("DatromaxMiner -- runtime error: %s\n", e.what());
        return;
    }
}

int GenerateDatromaxs(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++){
        minerThreads->create_thread(boost::bind(&DatromaxMiner, boost::cref(chainparams)));
    }
    return(numCores);
}
