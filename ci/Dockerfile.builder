...
# datromax_hash
RUN git clone https://github.com/datromax/datromax_hash
RUN cd datromax_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} datromax
RUN useradd -u ${USER_ID} -g datromax -s /bin/bash -m -d /datromax datromax
...
RUN mkdir /datromax-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /datromax-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /datromax-src
